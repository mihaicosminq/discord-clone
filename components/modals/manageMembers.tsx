...
const roleIconMap = {
    "GUEST": null,
    "MODERATOR": <Shield className="h-4 w-4 ml-2 text-purple-950"/>,
        ...
        <Dialog open={isModalOpen} onOpenChange={onClose}>
            <DialogContent className="bg-white text-black overflow-hidden">
                <DialogHeader className="pt-8 px-6">
                    ...
                    <DialogDescription
                        className="flex font-bold text-zinc-600"><Users
                        className="w-4 h-4 text-pink-950"/>{server?.members?.length}</DialogDescription>
                </DialogHeader>
                <ScrollArea className="mt-8 max-h-[420px] pr-6">
                    {server && server?.members?.map((member) => (
